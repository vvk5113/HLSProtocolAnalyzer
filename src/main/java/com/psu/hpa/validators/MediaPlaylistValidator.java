...
public class MediaPlaylistValidator {
	...
	public void validate(List<String> contentList, FileWriter fileWriter, String mediaPlaylistURI, String masterStreamURI) throws IOException {
		...
		if(CollectionUtils.isNotEmpty(contentList)) {
			if(!(contentList.get(0).equals(Constants.EXTM3U))) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.INCORRECT_FORMAT, mediaPlaylistURI, errorDetails);
			}
			...
			if(CommonUtils.hasDuplicate(contentList, Constants.EXTM3U)) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 2, ErrorType.DUPLICATE_TAG, mediaPlaylistURI, errorDetails);
			}
			...
			if(!matchEXTM3U.matches()) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 3, ErrorType.INCORRECT_FORMAT, mediaPlaylistURI, errorDetails);
			}
			...
			if(!(StringUtils.join(contentList).contains(Constants.EXT_X_VERSION))) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 4, ErrorType.MISSING_TAG, mediaPlaylistURI, errorDetails);
			}
			...
			if(CommonUtils.hasDuplicate(contentList, Constants.EXT_X_VERSION)) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.DUPLICATE_TAG, mediaPlaylistURI, errorDetails);
			}
			...
			if(!(StringUtils.join(contentList).contains(Constants.EXT_X_TARGETDURATION))) {
				...
				CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.MISSING_TAG, mediaPlaylistURI, errorDetails);
			}
			...
			for(int i=0; i<contentList.size(); i++) {
				...
				if(lineContent.contains(Constants.EXT_X_VERSION) && !hasDuplicate_EXT_X_VERSION) {
					validate_EXT_X_VERSION(contentList, lineContent, fileWriter, mediaPlaylistURI);
				}
				...
				if(lineContent.contains(Constants.EXT_X_TARGETDURATION)) {
					...
					if(!matchEXT_X_TARGETDURATION.matches()) {
						...
						CommonUtils.writeToCSVFile(fileWriter, i, ErrorType.INCORRECT_FORMAT, mediaPlaylistURI, errorDetails);
					}
				}
				...
				if(lineContent.contains(Constants.EXTINF)) {
					...
					if(contentList.get(i+1).startsWith("#") || !contentList.get(i+1).endsWith(".ts")) {
						...
						CommonUtils.writeToCSVFile(fileWriter, i, ErrorType.MISSING_TAG, mediaPlaylistURI, errorDetails);
					} else {
				...
				if(!lineContent.startsWith("#") && lineContent.endsWith(".ts")) {
					...
					try {
						...
						CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.MISSING_STREAM_FILE, mediaPlaylistURI, errorDetails);
	...
	private void validate_EXT_X_VERSION(List<String> contentList, String element, FileWriter fileWriter, String mediaPlaylistURI) throws IOException {
			...
			for(String lineContent : contentList) {
				if(lineContent.contains(Constants.EXT_X_KEY)) {
					if(!(EXT_X_VERSION_VALUE1 >= 2)) {
						...
						CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.FUNCTIONAL_TAG, mediaPlaylistURI, errorDetails);
					}
				}
				if(lineContent.contains(Constants.EXTINF)) {
					...
					if((EXT_X_VERSION_VALUE1 >= 3) && !(EXTINFDuration.contains("."))) {
						String errorDetails = "Media Playlist must include all EXTINF duration values as Floating-point values if "+Constants.EXT_X_VERSION+" value is 3 or higher.";
						CommonUtils.writeToCSVFile(fileWriter, 1, ErrorType.FUNCTIONAL_TAG, mediaPlaylistURI, errorDetails);
					}
				}
